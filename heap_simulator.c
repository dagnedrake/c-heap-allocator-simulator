...
/* Heap Simulator
   init()  initialize the simulated heap
   myMalloc()  allocate a memory block
   myFree()  free memory
...
void addBlock(int loc, int len) { 
  ...
  //mark begin & end of allocated block
...
int traverse(int len)
{
    ...
    //what if end of memPool reached and didn't find anything?
    if (loc == SIZE)
    {
        return -1;
    }
...
void myFree (int* p) { 
    ...
    /*initialize new start location and size
    ...
    markBlockPtr(newStart, newSize, false); //markBlock change start to free
